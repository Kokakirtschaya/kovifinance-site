...
# Идентификатор сборки. Задаём здесь, в окружении ОДНОГО процесса сборки:
# Next вшивает переменные NEXT_PUBLIC_* в код на этапе next build, поэтому и
# клиентский бандл, и серверный /api/build получают одно и то же значение, а
# каждая новая сборка — новое. По нему открытая вкладка понимает, что версия
# на сервере сменилась, и обновляется сама (components/site/ErrorGuard.tsx).
# ⚠️ Вычислять это в next.config.ts нельзя: конфиг читается и при старте
# контейнера, значение разошлось бы со сборкой и дало бы вечную перезагрузку.
RUN export NEXT_PUBLIC_BUILD_ID="$(date +%s)" && npm run build
